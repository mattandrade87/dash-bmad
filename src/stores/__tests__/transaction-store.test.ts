...
describe("TransactionStore", () => {
  ...
  it("deve adicionar transação", () => {
    ...
    expect(useTransactionStore.getState().transactions[0]).toEqual(
      newTransaction
    );
  });
  ...
  it("deve deletar transação", () => {
    const { addTransaction, deleteTransaction } =
      useTransactionStore.getState();
  ...
  it("deve filtrar transações por tipo", () => {
    const { setTransactions, setFilters, getFilteredTransactions } =
      useTransactionStore.getState();
